...
export default function SettingsSec({
    ...
    if (existing) {
      updatedList = classStaffs.map(cs => {
        if (cs.classId === classId) {
          return {
            ...
            guruKelasNuptk: roleType === 'wali' ? nuptk : (roleType === 'guru' ? nuptk : cs.guruKelasNuptk)
          };
        }
        return cs;
      });
      ...
      updatedList = [
        ...classStaffs,
        {
          ...
          guruKelasNuptk: roleType === 'wali' ? nuptk : (roleType === 'guru' ? nuptk : '')
        }
      ];
    }
  ...
  const handleSaveSystem = (e: React.FormEvent) => {
    ...
    toast.success('Pengaturan parameter sistem berhasil diperbarui dan disinkronkan!');
  };

  // Convert and compress uploaded logo to compact Base64
  ...
  const handleAddYear = () => {
    ...
    if (academicSetting.years.includes(val)) {
      toast.warning('Tahun Pelajaran sudah terdaftar.');
      return;
    }
    onUpdateAcademic({
      ...academicSetting,
  ...
  const handleAddSubject = () => {
    ...
    if (academicSetting.subjects.includes(val)) {
      toast.warning('Mata Pelajaran sudah terdaftar.');
      return;
    }
    onUpdateAcademic({
      ...academicSetting,
  ...
  const handleAddClass = () => {
    ...
    if (academicSetting.classes.includes(val)) {
      toast.warning('Rombel/Kelas sudah terdaftar.');
      return;
    }
    onUpdateAcademic({
      ...academicSetting,
  ...
  const handleDeleteYear = async (yearName: string) => {
    if (academicSetting.years.length <= 1) {
      toast.error('Harus ada minimal satu Tahun Pelajaran.');
      return;
    }
    ...
    const confirmed = await showConfirm(`Apakah Anda yakin ingin menghapus Tahun Pelajaran ${yearName}?`, {
  ...
  const handleDeleteSubject = async (subjectName: string) => {
    if (academicSetting.subjects.length <= 1) {
      toast.error('Harus ada minimal satu Mata Pelajaran.');
      return;
    }
    const confirmed = await showConfirm(`Apakah Anda yakin ingin menghapus Mata Pelajaran ${subjectName}?`, {
  ...
  const handleDeleteClass = async (className: string) => {
    if (academicSetting.classes.length <= 1) {
      toast.error('Harus ada minimal satu Rombel/Kelas.');
      return;
    }
    const confirmed = await showConfirm(`Apakah Anda yakin ingin menghapus Rombel/Kelas ${className}?`, {
  ...
  const handleAddExclusion = () => {
    if (!exclusionClass || !exclusionSubject) {
      toast.warning('Silakan pilih kelas dan mata pelajaran.');
      return;
    }
    ...
    if (isExist) {
      toast.warning('Pengecualian mata pelajaran ini untuk kelas tersebut sudah didaftarkan.');
      return;
    }
  ...
  const handleDeleteExclusion = async (className: string, subject: string) => {
    const confirmed = await showConfirm(`Apakah Anda yakin ingin menghapus pengecualian mapel ${subject} di kelas ${className}?`, {
      title: 'Hapus Pengecualian Mapel',
      type: 'danger'
    });
    if (confirmed) {
      const currentExclusions = academicSetting.subjectExclusions || [];
  ...
  const handleAddGuruSubmit = (e: React.FormEvent) => {
    ...
    if (!guruNuptk.trim() || !guruName.trim() || !guruEmail.trim() || !guruUsername.trim()) {
      toast.warning('Harap lengkapi seluruh kolom isian Guru.');
      return;
    }
    if (teachers.some(t => t.nuptk === guruNuptk)) {
      toast.error('NUPTK Guru tersebut sudah terdaftar.');
      return;
    }
  ...
  const handleSaveEditGuru = (e: React.FormEvent) => {
    ...
    if (!editName.trim() || !editEmail.trim() || !editUsername.trim()) {
      toast.warning('Harap lengkapi nama, email, dan username Guru.');
      return;
    }
    ...
    toast.success('Identitas guru berhasil diperbarui!');
  };
  ...
  const handleSaveResetPassword = (e: React.FormEvent) => {
    ...
    if (!newPassword.trim()) {
      toast.warning('Harap masukkan kata sandi baru.');
      return;
    }
    ...
    setNewPassword('');
    toast.success('Kata sandi guru berhasil diubah!');
  };
                      ...
                      <tr key={cls} className="hover:bg-slate-850/20 transition-all duration-150">
                        ...
                        <td className="p-3">
                          <select
                            value={selectedWali}
                            disabled
                            className="bg-slate-900 border border-slate-800 rounded-lg p-2 text-slate-500 focus:outline-none text-xs w-full max-w-xs font-medium cursor-not-allowed opacity-70"
                          >
                            <option value="">-- Otomatis Mengikuti Wali Kelas --</option>
                            {teachers.map(t => (
                              <option key={t.nuptk} value={t.nuptk}>
                                {t.name} ({t.subject})
                              </option>
                      ...
                      <button
                        ...
                        onClick={async () => {
                          if (onDeletePeriodData) {
                            ...
                            toast.error('Aksi hapus data semester tidak dikonfigurasi dengan benar.');
                          }
                        }}
                    ...
                    <tr key={t.nuptk} className="hover:bg-slate-800/20 transition duration-100">
                      ...
                      <td className="p-4 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <button
                            onClick={async () => {
                              const confirmed = await showConfirm(`Hapus guru ${t.name}? Tindakan ini akan membatalkan hak akses login guru.`, {
